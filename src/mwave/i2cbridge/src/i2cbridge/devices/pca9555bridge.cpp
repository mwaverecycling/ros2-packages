...
namespace I2CBridge
{
    PCA9555Bridge::PCA9555Bridge(const mwave_messages::msg::I2CDevice& config, rclcpp::Node* node)
    {
        ...
        this->device_ref->write_output(0);
        ...
        RCLCPP_INFO(node->get_logger(), "    Setup %s(0x%02x, 0x%02x) device topics", config.device.c_str(), config.bus, config.address);
        ...
        // TODO: Combine devices with common sample rate into one timer
